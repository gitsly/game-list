...
;; - chat: documents created automatically, one per chat-session
...
(defn create-default-chat-session
  [session-name]
  {:session session-name
   :entries []})

(defn chat-session-or-default
  [session-name]
  (let [found (mc/find-one-as-map (connect) "chat" {:session session-name})]
    (if found
      found ; return old
      (mc/insert-and-return (connect) "chat" (create-default-chat-session session-name)))))
...
(defn add-chat-entry
  ...
  (let [chat-id nil ; TODO: if multiple chats
        chat (chat-session-or-default session)
        entry-oid (ObjectId.)
        chat-oid (-> chat :_id)
        entry-with-id (assoc entry :_id entry-oid)]
    (mc/update (connect) "chat" {:_id chat-oid} {$push {:entries entry-with-id}})
